...
**`KC_HOSTNAME` must match `--oidc-issuer-url` byte-for-byte.** [manifests/31-keycloak.yaml](keycloak-kubelogin-rbac/manifests/31-keycloak.yaml) sets `KC_HOSTNAME=http://keycloak.k8s.lab` — Keycloak puts that into OIDC discovery as `issuer`, and the API server compares the JWT `iss` claim against `--oidc-issuer-url` exactly. A trailing slash, port, or `http`/`https` mismatch breaks login with `oidc: issuer did not match the issuer claim`.

**Versions.** bitnami/openldap `2.6.10`, postgres `16-alpine`, quay.io/keycloak/keycloak `26.6`, ingress-nginx `controller-v1.15.1` (final release; project archived 2026-03-24, same constraint as `ingress-nginx-w-certificate/`). Bumping requires editing the manifest line directly — no shared variable.
